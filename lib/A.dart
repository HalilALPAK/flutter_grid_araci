...
// Yeni: Widget türlerini tanımlayan enum
enum WidgetType { text, image, none }
...
class _GridSelectionScreenState extends State<GridSelectionScreen> {
    ...
    final List<List<CellData>> newGridData = List.generate(
      ...
      (row) => List.generate(gridCols, (col) {
        ...
        return CellData(
          ...
          isMerged: false,
          mergedCells: [],
          widgetType: WidgetType.none, // Yeni eklenen
          textContent: null, // Yeni eklenen
          imageUrl: null, // Yeni eklenen
          borderRadius: null, // Yeni eklenen
          borderWidth: null, // Yeni eklenen
          borderColor: null, // Yeni eklenen
        );
      }),
    );
    ...
    for (var row in currentGrid) {
      for (var cell in row) {
        if (!cell.isFilled &&
            ...
            cell.widgetType == WidgetType.none) {
  ...
  void _updateDefaultEmptyCellColorInCellChildren(List<CellData> children) {
    for (var childCell in children) {
      if (!childCell.isFilled &&
          ...
          childCell.widgetType == WidgetType.none) {
  ...
  void _clearAllSelectionsAndSplits() {
    setState(() {
      ...
      isMergeMode = false;
      selectedMergePaths = [];
      isAddWidgetMode = false; // Widget ekleme modunu da sıfırla
    });
  }
  ...
  void _onCellTap(List<int> path, BuildContext scaffoldContext) {
    ...
    setState(() {
      if (isAddWidgetMode) {
        // Widget ekleme modunda hücreye tıklanınca dialog aç
        if (targetCell.splitChildren.isNotEmpty) {
          ScaffoldMessenger.of(scaffoldContext).showSnackBar(
            const SnackBar(
              content: Text(
                'Bölünmüş alanlara doğrudan widget ekleyemezsiniz. Alt parçayı seçin.',
              ),
        ...
        _showAddWidgetDialog(scaffoldContext, path, targetCell);
        isAddWidgetMode = false; // Dialog açıldıktan sonra modu kapat
        return;
      }
      ...
      if (isMergeMode) {
        ...
        if (targetCell.isMerged &&
            (targetCell.mergedCells.isEmpty ||
                targetCell.mergedCells.first[0] != path[0] ||
                targetCell.mergedCells.first[1] != path[1])) {
          // Eğer tıklanan hücre birleştirilmiş bir alanın parçası ise ama ana hücre değilse, uyarı ver.
          ScaffoldMessenger.of(scaffoldContext).showSnackBar(
            const SnackBar(
              content: Text('Bu alan zaten birleştirilmiş bir alanın parçası.'),
              duration: Duration(seconds: 2),
            ),
          );
          return;
        }
        // Eğer birleştirilmiş bir alanın ana hücresi ise, seçimi kaldır veya uyar
        if (targetCell.isMerged &&
            targetCell.mergedCells.isNotEmpty &&
            targetCell.mergedCells.first[0] == path[0] &&
            targetCell.mergedCells.first[1] == path[1]) {
          ScaffoldMessenger.of(scaffoldContext).showSnackBar(
            const SnackBar(
              content: Text(
                'Birleştirilmiş alanı seçimi kaldırmak için "Seçimi Bitir ve Birleştir" butonuna tekrar basın.',
              ),
              duration: Duration(seconds: 2),
            ),
          );
      ...
      if (targetCell.isMerged) {
        ...
        for (var p in targetCell.mergedCells) {
          ...
          if (cellToUpdate != null) {
            ...
            cellToUpdate.widgetType = WidgetType.none; // Widget'ı kaldır
          }
        }
        ...
        targetCell.widgetType = WidgetType.none; // Widget'ı kaldır
      } else {
        ...
        if (targetCell.isFilled && targetCell.color != selectedColor) {
          ...
          targetCell.widgetType = WidgetType.none; // Widget'ı kaldır
            ...
            targetCell.widgetType != WidgetType.none) {
          targetCell.isFilled = true;
          targetCell.color = selectedColor;
          targetCell.widgetType = WidgetType.none; // Widget'ı kaldır
        } else {
  ...
  // Yeni: Widget Ekleme Diyaloğu
  void _showAddWidgetDialog(
    BuildContext context,
    List<int> path,
    CellData cell,
  ) {
    WidgetType currentWidgetType = cell.widgetType ?? WidgetType.none;
    ...
    double currentBorderWidth = cell.borderWidth ?? 0.0;
    Color currentBorderColor = cell.borderColor ?? Colors.black;
    double currentBorderRadius =
        (cell.borderRadius != null && cell.borderRadius is BorderRadius)
            ? (cell.borderRadius as BorderRadius)
                .topLeft
                .x // Varsayılan olarak topLeft'ı al
            : 0.0;
      ...
      builder: (BuildContext dialogContext) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return AlertDialog(
              title: const Text('Hücreye Widget Ekle'),
                  ...
                  children: [
                    DropdownButton<WidgetType>(
                      ...
                      onChanged: (WidgetType? newValue) {
                        setDialogState(() {
                          currentWidgetType = newValue ?? WidgetType.none;
                        });
                      },
                      items: const [
                        DropdownMenuItem(
                          value: WidgetType.none,
                          child: Text('Boş (Renk)'),
                        ),
                        DropdownMenuItem(
                    ...
                    if (currentWidgetType == WidgetType.text)
                      TextField(
                    ...
                    if (currentWidgetType == WidgetType.image)
                      TextField(
                    ...
                    Text(
                      'Kenarlık Kalınlığı: ${currentBorderWidth.toStringAsFixed(1)}',
                    ),
                    Slider(
                      ...
                      max: 10.0,
                      divisions: 20,
                      label: currentBorderWidth.toStringAsFixed(1),
                    ...
                    const SizedBox(height: 20),
                    Text(
                      'Köşe Yuvarlama: ${currentBorderRadius.toStringAsFixed(1)}',
                    ),
                    Slider(
                      value: currentBorderRadius,
                      min: 0.0,
                      max: 50.0, // Max yuvarlama değeri
                      divisions: 50,
                      label: currentBorderRadius.toStringAsFixed(1),
                      onChanged: (double value) {
                        setDialogState(() {
              ...
              actions: <Widget>[
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    setState(() {
                      cell.widgetType = currentWidgetType;
                      cell.textContent =
                          currentWidgetType == WidgetType.text
                              ? textController.text
                              : null;
                      cell.imageUrl =
                          currentWidgetType == WidgetType.image
                              ? imageUrlController.text
                              : null;

                      // Widget eklendiğinde hücre rengini şeffaf yap
                      if (currentWidgetType != WidgetType.none) {
                        cell.color = Colors.transparent;
                        cell.isFilled = false;
                      } else {
                        // Widget kaldırıldığında varsayılan boş rengi geri getir
                        cell.color = _defaultEmptyCellColor;
                        cell.isFilled = false;
                      }

                      cell.borderWidth = currentBorderWidth;
                      cell.borderColor = currentBorderColor;
                      cell.borderRadius = BorderRadius.circular(
                        currentBorderRadius,
                      );
                    });
                    Navigator.of(dialogContext).pop();
  ...
  void _onPanStart(
    ...
    if (selectedSplitMode != 'none' || isMergeMode || isAddWidgetMode) {
      ScaffoldMessenger.of(scaffoldContext).showSnackBar(
        const SnackBar(
          content: Text(
            'Bölme, birleştirme veya widget ekleme modunda sürükleme yapılamaz.',
          ),
    ...
    if (cell.splitChildren.isNotEmpty ||
      ...
      ScaffoldMessenger.of(scaffoldContext).showSnackBar(
        const SnackBar(
          content: Text(
            'Bölünmüş, birleştirilmiş veya widget içeren parçalar sürüklenemez.',
          ),
          duration: Duration(seconds: 2),
        ),
      );
    ...
    setState(() {
      ...
      cell.widgetType = WidgetType.none; // Sürüklerken widget'ı kaldır
      cell.borderRadius = null;
      cell.borderWidth = null;
  ...
  void _performSplit(CellData cell, String mode) {
      ...
      children.add(
        CellData(
          ...
          color:
              cell.color == Colors.transparent
                  ? _defaultEmptyCellColor
                  : cell.color, // Bölündüğünde şeffaf olmasın
          splitChildren: [],
          splitDirection: null,
          splitRatio: null,
          widgetType:
              WidgetType.none, // Yeni bölünmüş hücrelerde widget olmasın
          textContent: null,
          imageUrl: null,
          borderRadius: null,
          borderWidth: null,
    ...
    cell.isFilled = false;
    cell.color = Colors.transparent; // Ana hücre şeffaf olur
    cell.widgetType =
        WidgetType.none; // Bölünen hücrenin kendisinde widget kalmasın
    cell.borderRadius = null;
    cell.borderWidth = null;
    cell.borderColor = null;
  }
  ...
  void _performMerge(BuildContext scaffoldContext) {
    if (selectedMergePaths.isEmpty) {
      ...
      if (isMergeMode) {
        // Sadece aktif olarak birleştirme modundan çıkarken bu uyarıyı ver
        ScaffoldMessenger.of(scaffoldContext).showSnackBar(
          const SnackBar(
    ...
    for (var path in selectedMergePaths) {
      if (path.length != 2) {
        // Bu kontrol _onCellTap içinde de var, burada tekrar emin olmak için.
        ScaffoldMessenger.of(scaffoldContext).showSnackBar(
          const SnackBar(
            content: Text(
              'Sadece en üst seviye hücreleri birleştirebilirsiniz.',
    ...
    for (int r = minRow; r <= maxRow; r++) {
      for (int c = minCol; c <= maxCol; c++) {
        // [r, c]'nin selectedMergePaths içinde olup olmadığını kontrol etmek için List'i String'e çevirip karşılaştırabiliriz
        // çünkü List'ler referans karşılaştırması yapar.
        if (!selectedMergePaths.any((p) => p[0] == r && p[1] == c)) {
          ScaffoldMessenger.of(scaffoldContext).showSnackBar(
            const SnackBar(
    ...
    // Seçilen hücrelerden herhangi birinin bölünmüş veya widget içeren olup olmadığını kontrol et
    for (var path in selectedMergePaths) {
      CellData? cell = _findCellByPath(path, gridData);
      if (cell != null &&
          (cell.splitChildren.isNotEmpty ||
              cell.widgetType != WidgetType.none)) {
        ScaffoldMessenger.of(scaffoldContext).showSnackBar(
          const SnackBar(
            content: Text(
              'Seçilen alanlar bölünmüş hücreler veya widget içeriyor. Birleştirme yapılamaz.',
            ),
            duration: Duration(seconds: 2),
          ),
        );
    ...
    setState(() {
      Color mergedColor = Colors.transparent;
      if (selectedMergePaths.isNotEmpty) {
        mergedColor =
      ...
      for (int r = minRow; r <= maxRow; r++) {
        for (int c = minCol; c <= maxCol; c++) {
          ...
          cell.splitRatio = null;
          cell.widgetType =
              WidgetType.none; // Birleştirilen hücrelerde widget olmasın
          ...
          if (r == minRow && c == minCol) {
            ...
            cell.isFilled = true;
            cell.mergedCells = List.from(selectedMergePaths); // Deep copy
          } else {
            cell.isFilled = false;
            cell.color = Colors.transparent;
            cell.mergedCells = [];
  ...
  String _generateFlutterCode() {
    ...
    String _generateCellCode(CellData cell) {
      String childWidgetCode;
      if (cell.widgetType == WidgetType.text) {
        childWidgetCode = '''
            Center(
              child: Text(
                '${cell.textContent ?? ''}',
                ...
                style: TextStyle(
                  color: ${cell.color.computeLuminance() > 0.5 ? 'Colors.black' : 'Colors.white'},
                  fontSize: 16,
                  fontWeight: FontWeight.bold,
                ),
              ),
          ...
          ''';
      } else if (cell.widgetType == WidgetType.image) {
        childWidgetCode = '''
            Image.network(
              '${cell.imageUrl ?? ''}',
              fit: BoxFit.cover,
          ...
          ''';
      } else if (cell.splitChildren.isEmpty) {
        // Normal boş veya renkli hücre
        childWidgetCode = '''
          Center(
            child: Text(
              // '${cell.isFilled ? 'Dolu' : 'Boş'}', // Gösterim için
              '',
              style: TextStyle(
                color: ${cell.color.computeLuminance() > 0.5 ? 'Colors.black87' : 'Colors.white'},
                fontSize: 9,
              ),
            ),
          )
        ...
        // Bölünmüş hücrenin içinde çocukları var
        String childrenCode = '';
      ...
      String borderRadiusCode =
          cell.borderRadius != null && cell.borderRadius is BorderRadius
              ? 'BorderRadius.circular(${(cell.borderRadius as BorderRadius).topLeft.x})'
              : 'BorderRadius.circular(4)'; // Default if null

      String borderWidthCode =
          cell.borderWidth != null && cell.borderWidth! > 0
              ? 'width: ${cell.borderWidth!.toStringAsFixed(1)},'
              : 'width: 1.0,';

      String borderColorCode =
          cell.borderColor != null
              ? 'color: ${colorToCode(cell.borderColor!)},'
              : 'color: Colors.blueGrey.shade200,';

      return '''
        Container(
          ${cell.color == Colors.transparent ? '' : 'color: ${colorToCode(cell.color)},'}
          margin: const EdgeInsets.all(2.0),
          decoration: BoxDecoration(
            ${cell.color == Colors.transparent ? '' : 'color: ${colorToCode(cell.color)},'} // Eğer şeffaf değilse
            border: Border.all(
              $borderColorCode
              $borderWidthCode
            ),
            borderRadius: $borderRadiusCode,
          ),
          child: ${cell.splitChildren.isEmpty ? childWidgetCode : childWidgetCode},
        )
      ''';
    }
              ...
              return GestureDetector(
                onTap: () {
                  setState(() {
                    ...
                    selectedMergePaths = [];
                    isAddWidgetMode =
                        false; // Renk seçince widget ekleme modunu kapat
                  });
                },
                child: Container(
                  width: 40,
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              ElevatedButton.icon(
                                  ...
                                  setState(() {
                                    ...
                                    if (isAddWidgetMode) {
                                      ScaffoldMessenger.of(
                                        ...
                                        const SnackBar(
                                          content: Text(
                                            'Widget eklemek için bir hücreye tıklayın.',
                                          ),
                                          duration: Duration(seconds: 2),
                                        ),
                                      );
                                ...
                                icon: Icon(
                                  isAddWidgetMode ? Icons.close : Icons.widgets,
                                ),
                                label: Text(
                                  isAddWidgetMode
                                      ? 'Widget Ekleme Modunu Kapat'
                                      : 'Hücreye Widget Ekle',
                                ),
                                style: ElevatedButton.styleFrom(
                                  backgroundColor:
                              ...
                              ElevatedButton.icon(
                                onPressed: () {
                                  setState(() {
                                    ...
                                    isAddWidgetMode =
                                        false; // Widget ekleme modunu kapat
                                    if (isMergeMode) {
                                      selectedMergePaths =
  ...
  Widget _buildCellWidget(
    ...
    bool isSelectedForWidget =
        isAddWidgetMode; // Widget ekleme modunda tıklanmayı bekleyen hücreleri görsel olarak belirtmek için
    ...
    // Hücrenin içeriği (widget veya boş)
    Widget? cellContent;
    if (cellData.widgetType == WidgetType.text) {
      cellContent = Center(
        child: Text(
    ...
    } else if (cellData.widgetType == WidgetType.image) {
      cellContent =
          cellData.imageUrl != null && cellData.imageUrl!.isNotEmpty
              ? Image.network(
                cellData.imageUrl!,
                fit: BoxFit.cover,
                errorBuilder:
    ...
    } else if (cellData.splitChildren.isNotEmpty) {
      // Bölünmüş hücrelerin içeriği, alt hücreleri barındıracak Column/Row yapısıdır.
      cellContent =
                      ...
                      return Expanded(
                        flex:
                            cellData.splitRatio != null &&
                                    childIndex < cellData.splitRatio!.length
                                ? cellData.splitRatio![childIndex]
                                : 1,
                        child: Padding(
                          padding: const EdgeInsets.all(2.0),
                          child: _buildCellWidget(childCell, [
                            ...path,
                            childIndex,
                          ], scaffoldContext),
                        ),
      ...
      // Boş veya renkli hücre
      cellContent = Center(
        child: Text(
          path.map((e) => e.toString()).join(','),
          style: TextStyle(
            color:
                (cellData.isFilled ? cellData.color : _defaultEmptyCellColor)
                            .computeLuminance() >
                        0.5
                    ? Colors.black87
                    : Colors.white,
    ...
    return GestureDetector(
      onTap: () => _onCellTap(path, scaffoldContext),
      onPanStart:
          selectedSplitMode != 'none' ||
                  isMergeMode ||
                  isAddWidgetMode ||
              ...
              ? null
              : (details) =>
                  _onPanStart(details, path[0], path[1], scaffoldContext),
      onPanUpdate:
          selectedSplitMode != 'none' ||
                  isMergeMode ||
                  isAddWidgetMode ||
              ...
              ? null
              : (details) =>
                  _onPanUpdate(details, path[0], path[1], scaffoldContext),
      onPanEnd:
          selectedSplitMode != 'none' ||
                  isMergeMode ||
                  isAddWidgetMode ||
          ...
          border: Border.all(
            color:
                isSelectedForMerge
                    ...
                    : isAddWidgetMode // Widget ekleme modunda vurgu
                    ? Colors.purple.shade400
                    : (selectedSplitMode != 'none'
                        ? Colors
                            .deepOrange // Bölme modunda vurgu
                        : (cellData.borderWidth != null &&
                                cellData.borderWidth! > 0
                            ? cellData.borderColor ??
                                Colors
                                    .black // Özel kenarlık rengi
                            : (cellData
                                    .isFilled // Renkliyse veya widget varsa
                                ? Colors.blueAccent
                                : (Theme.of(scaffoldContext).brightness ==
                                        Brightness.light
            ...
            width:
                isSelectedForMerge
                    ...
                    : (selectedSplitMode != 'none'
                        ? 3.0
                        : (cellData.borderWidth ??
                            (cellData.isFilled ? 2.0 : 1.0))),
          ),
          borderRadius: cellData.borderRadius ?? BorderRadius.circular(4),
        ),
        child: cellContent, // Dinamik içerik burada
      ),
    );
  }
}
...
class CellData {
  ...
  // Yeni eklendi: Widget özellikleri
  WidgetType? widgetType;
  String? textContent;
  String? imageUrl;

  // Yeni eklendi: Kenarlık ve köşe yuvarlama
